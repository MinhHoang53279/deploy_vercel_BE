...
- Course thumbnails can use normal `https://...` image URLs, so backend deployment does not depend on file upload for seeded demo courses
- The current upload implementation stores files on the local filesystem, which is fine for local development but not durable on Vercel; for deployment, prefer external image URLs for now
